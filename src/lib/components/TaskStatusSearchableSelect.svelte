...
<script lang="ts">
    ...
    export type TaskStatusSelectItem = {
        id: string
        title: string
        is_initial?: boolean
        is_terminal?: boolean
    ...
    function toStatusRecord(item: TaskStatusSelectItem): TaskStatusRecord {
        return {
            id: item.id,
            name: item.title,
            description: null,
            ...
            kanban_pos_x: 0,
            kanban_pos_y: 0,
            is_initial: item.is_initial ?? false,
    ...
    const selectedBorderColor = $derived(
        selectedStatus?.color != null ? mixDisplayColorInt(selectedStatus.color) : null,
    )

    const filtered = $derived(
        query.trim().length === 0 ? statuses : fuse.search(query).map((result) => result.item),
    )

    const visibleCount = $derived(Math.ceil(viewportHeight / ITEM_HEIGHT) + 2)

    interface VirtualItem extends TaskStatusSelectItem {
        __offset: number
    }
...
<div class="relative space-y-1" onkeydown={onKeydown} tabindex="-1">
    ...
    >
        {#if selectedStatus}
            <TaskStatusRefCard
                    status={toStatusRecord(selectedStatus)}
                    fullWidth
